...
PImage[] imagenes; // Arreglo para almacenar las imágenes de cada pantalla

void setup() {
  size(640, 480);
  lastChangeTime = millis(); // Inicializa el tiempo del último cambio

  // Cargar imágenes (asegúrate de colocar las imágenes en la carpeta "data" del sketch)
  imagenes = new PImage[totalScreens];
  imagenes[1] = loadImage("jardin de historias.jpg");
  imagenes[2] = loadImage("jardin de historias 2.jpg");
}
...
void draw() {
  ...
  if (screen < totalScreens) {
    ...
    // Mostrar imagen con transparencia
    if (imagenes[screen] != null) {
      tint(255, alpha);
      image(imagenes[screen], 0, 0, width, height);
      noTint(); // Restablecer tint
    }
